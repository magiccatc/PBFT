...
func genRsaKeys() {
	if !isExist("../public/Keys") {
		...
		err := os.Mkdir("../public/Keys", 0644) // 如果不存在，创建keys文件夹
		if err != nil {
			fmt.Println("ryw pk sk文件没创建成功1111111 ...")
			log.Panic()
		}
		for i := 0; i <= nodeCount; i++ { // 创建keys文件夹下的五个子目录N0到N nodeCount
			if !isExist("../public/Keys/N" + strconv.Itoa(i)) {
				err := os.Mkdir("../public/Keys/N"+strconv.Itoa(i), 0644)
				if err != nil {
					fmt.Println("ryw pk sk文件没创建成功 ...")
					log.Panic()
			...
			if err != nil {
				fmt.Println("ryw pk sk私钥文件没有打开成功")
				log.Panic(err)

			}
			defer file.Close()
			file.Write(priv)
			//fmt.Println("ryw文件中内容", priv)
			...
			if err != nil {
				fmt.Println("ryw pk sk公钥文件没有打开成功")
				log.Panic(err)
