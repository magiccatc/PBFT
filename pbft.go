...
// func (p *pbft) handleRequest(data []byte, startTime time.Time) {
func (p *pbft) handleRequest(data []byte) {
	...
	case cCommit:
		// p.handleCommit(content, startTime)
...
func (p *pbft) handleClientRequest(content []byte) {
	...
	//添加信息序号
	p.sequenceIDAdd() // 也就是值加1
	//获取消息摘要
	digest := getDigest(*r) // 得到的是hash值的字符串
...
func (p *pbft) handlePrePrepare(content []byte) {
	...
	if digest := getDigest(pp.RequestMessage); digest != pp.Digest {
		...
		// // 获取当前时间并传递给handleRequest
		// currentTime := time.Now()
		// p.handleRequest(content, currentTime)
	}
}
...
// 处理提交确认消息
// func (p *pbft) handleCommit(content []byte, t time.Time) {
func (p *pbft) handleCommit(content []byte) {
	...
	if _, ok := p.prePareConfirmCount[c.Digest]; !ok {
		...
		if count >= nodeCount/3*2 && !p.isReply[c.Digest] && p.isCommitBordcast[c.Digest] {
			...
			fmt.Println("[]byte(p.messagePool[c.Digest].Content)=", len([]byte(p.messagePool[c.Digest].Content)))
			wtime, _ := writeData(p.db, strconv.Itoa(p.messagePool[c.Digest].ID), []byte(p.messagePool[c.Digest].Content))
			fmt.Println("写延时为：", wtime.Milliseconds())
			...
			tmp := time.Now().Sub(p.startTime)
			fmt.Println("reply完毕,用时" + fmt.Sprintf("%dms", tmp.Milliseconds()))
			replyInfo := fmt.Sprintf("把接受到的消息写入.db数据库延迟为%dms。reply完毕,用时%dms。%s", wtime.Milliseconds(), tmp.Milliseconds(), info)
			fmt.Println(replyInfo)

			fmt.Println("正在reply客户端 ...")
			tcpDial([]byte(replyInfo), p.messagePool[c.Digest].ClientAddr)
